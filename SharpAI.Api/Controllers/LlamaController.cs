...
namespace SharpAI.Api.Controllers
{
    ...
    public class LlamaController : ControllerBase
    {
        ...
        public async Task<ActionResult<LlamaModelFile?>> LoadModelSimpleAsync([FromQuery] string modelNameOrPath, [FromQuery] bool fuzzyMatch = true, [FromQuery] bool forceUnload = true, [FromQuery] bool tryMultimodal = true, [FromQuery] int maxTokens = 1024, [FromQuery] LlamaBackend backend = LlamaBackend.CUDA, CancellationToken ct = default)
        {
            if (this.Llama.IsModelLoaded)
            {
                if (forceUnload)
